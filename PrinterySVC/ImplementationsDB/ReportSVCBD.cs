...
{
    public class ReportSVCBD : IReportSVC
    {

        private AbstractDbContext context;

        public ReportSVCBD(AbstractDbContext context)
        {
        ...
        public void SaveProductPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Editions.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].EditionName;
                    table.Cell(i + 1, 2).Range.Text = products[i].CostEdition.ToString();
                }
        ...
        public List<RacksLoadViewModel> GetRacksLoad()
        {
            return context.Racks
                            .ToList()
                            .GroupJoin(
                                    context.RackMaterials
                                                .Include(r => r.Material)
                                                .ToList(),
                                    rack => rack,
                                    rackMaterial => rackMaterial.Rack,
                                    (rack, RackMaterialList) =>
            new RacksLoadViewModel
            {
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.RackName;
        ...
        public List<CustomerBookcingsModel> GetCustomerOrders(ReportBindingModel model)
        {
            return context.Bookings
                            .Include(rec => rec.Customer)
                            .Include(rec => rec.Edition)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new CustomerBookcingsModel
                            {
        ...
        public void SaveCustomerOrders(ReportBindingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].EditionName, fontForCells));
                table.AddCell(cell);
